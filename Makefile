...
audit-python:
	docker compose run --rm backend sh -c 'uv export --frozen --no-dev --no-hashes --output-file /tmp/requirements.txt && pip-audit --strict --requirement /tmp/requirements.txt'

audit-node:
	docker compose run --rm frontend npm audit --audit-level=high

scan-config:
	docker run --rm --volume "$(CURDIR):/workspace:ro" --workdir /workspace $(TRIVY_IMAGE) config --exit-code 1 --severity HIGH,CRITICAL .

scan-fs:
	docker run --rm --volume "$(CURDIR):/workspace:ro" --workdir /workspace $(TRIVY_IMAGE) fs --exit-code 1 --severity HIGH,CRITICAL --scanners vuln,secret .
...
scan-images: build-images
	@set -eu; work="$(CURDIR)/.trivy-work"; trap 'rm -rf "$$work"' EXIT; \
		...
		docker run --rm --volume "$$work:/scan:ro" $(TRIVY_IMAGE) image --input /scan/backend.tar --exit-code 1 --severity HIGH,CRITICAL; \
		docker run --rm --volume "$$work:/scan:ro" $(TRIVY_IMAGE) image --input /scan/frontend.tar --exit-code 1 --severity HIGH,CRITICAL
